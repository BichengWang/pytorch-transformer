...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        q = (
            self.q_linear(q)
            .reshape(q.shape[0], -1, self.heads, self.d_k)
            .transpose(1, 2)
        )
        k = (
            self.k_linear(k)
            .reshape(k.shape[0], -1, self.heads, self.d_k)
            .transpose(1, 2)
        )
        v = (
            self.v_linear(v)
            .reshape(v.shape[0], -1, self.heads, self.d_k)
            .transpose(1, 2)
        )
...
class Encoder(nn.Module):
    def __init__(
        ...
        self.encode_layers = nn.ModuleList(
            [EncoderLayer(heads, d_model, d_ff, dropout) for i in range(num_layes)]
        )
...
class Decoder(nn.Module):
    def __init__(
        ...
        self.embedding = nn.Embedding(vocab_size, d_model, pad_idx)
        self.position_encode = PositionEncoding(d_model, max_seq_len)
        self.decoder_layes = nn.ModuleList(
            [DecoderLayer(heads, d_model, d_ff, dropout) for i in range(num_layes)]
        )
...
class Transformer(nn.Module):
    ...
    def generate_mask(self, query, key, is_triu_mask=False):
        ...
        if is_triu_mask:
            ...
            dst_triu_mask = (
                dst_triu_mask.unsqueeze(0)
                .unsqueeze(1)
                .expand(batch, 1, seq_q, seq_k)
                .to(device)
            )
